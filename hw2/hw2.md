...
```
## Error: factor LocationNormalized has new levels Argyll & Bute, Arundel,
## Ashington, Bacup, Ballymena, Bilston, Bishop Auckland, Brigg, Cramlington,
## Crediton, Dymock, Egremont, Esher, Fradley Park, Glenrothes, Godstone,
## Heanor, Henlow, Liphook, Llandrindod Wells, Melksham, Oldbury, Oxford
## Airport, Paris Island Little, Pembroke, Prudhoe, Radstock, Redcar, Saffron
## Walden, Shaftesbury, Southend, Stanford-Le-Hope, Stone, Thorpe St. Andrew,
## Thurso, Tidworth, Totnes, Uttoxeter, Wareham, Wokingham
```
...
```
## Mean R.Squared:  0.1832 Mean Training MAE:  10143 Mean Test Set MAE:  10170
```
...
```
## Mean R.Squared:  0.1944 Mean Training MAE:  10037 Mean Test Set MAE:  10056
```
...
```
## Mean R.Squared:  0.1025 Mean Training MAE:  10565 Mean Test Set MAE:  10568
```
...
```
## Mean R.Squared:  0.238 Mean Training MAE:  9766 Mean Test Set MAE:  9800
```
...
```
## Mean R.Squared:  0.2451 Mean Training MAE:  9736 Mean Test Set MAE:  9786
```
...
```r
form <- LogSalary ~ SourceNameReduced + Category + ContractTime + ContractType + 
...
```
## Mean R.Squared:  0.303 Mean Training MAE:  9326 Mean Test Set MAE:  9435
```
...
```r
library("DAAG")
form <- formula(LogSalary ~ SourceNameReduced + Category + ContractType + ContractTime + 
    CategoryReduced:ContractTime + CategoryReduced:ContractType)
...
![plot of chunk daag](figure/daag1.png) 
...
![plot of chunk daag](figure/daag2.png) 
...
![plot of chunk daag](figure/daag3.png) 
...
# list is better. loc_frame <-
# data.frame(matrix(unlist(location_list),nrow=31762,byrow=T))
...
```
## [1] "London"       "North London"
```

```r

# Works for me. Returns a single pair of values, returns blanks if it
# doesn't find a location. Now to wrap it up in lapply

# data <-
# llply(data$LocationNormalized,findRegions,.progress=create_progress_bar('text'))
```
...
```r
# loc_rows <- nrow(location_matrix) Region <- character(length =
# nrow(data)) SubRegion <- character(length = nrow(data)) for (i in
# 1:nrow(data)){ indx <-
# grep(paste0('^',data$LocationNormalized[i],'$'),location_matrix) if
# (length(indx) > 0) { row_indx <- indx[1] %% loc_rows if (row_indx == 0)
# row_indx <- loc_rows Region[i] <- location_matrix[row_indx,2]
# SubRegion[i] <- location_matrix[row_indx,3] } else { Region[i] <- ' '
# SubRegion[i] <- ' ' } }

# data$Region <- factor(Region) data$SubRegion <- factor(SubRegion)
...
```r
# cat Location_Tree.csv | sed 's/~/,/g' | sed 's/'//g' >
# Location_Tree2.csv awk -f buildMergable.awk Location_Tree2.csv |sort -u
# -t , -k 1,1 >mergableTable.csv

location_df <- read.csv("~/Documents/GeneralAssembly/GADS4/hw2/mergableTable.csv", 
    head = F)
...
# Need to ditch the NAs before using the merged locations. There are only
# 8.
...
form <- LogSalary ~ RegionReduced + SubRegionReduced + SourceNameReduced + Category + 
    ContractTime + ContractType + CategoryReduced:ContractTime + CategoryReduced:ContractType + 
...
```
## Mean R.Squared:  0.3389 Mean Training MAE:  9036 Mean Test Set MAE:  9164
```
...
form <- LogSalary ~ RegionReduced + SubRegionReduced + Category + SourceNameReduced + 
    ContractTime + ContractType + CategoryReduced:ContractTime + CategoryReduced:ContractType + 
    ContractType:ContractTime
...
```
## Mean R.Squared:  0.3988 Mean Training MAE:  9956 Mean Test Set MAE:  9986
```
...
```
##  Mean Training MAE:  9036 Mean Test Set MAE:  9164
```
...
for (i in 1:max(data$fold)) {
    ...
    trainPredictions <- exp(predict(model, x, s = 5e-04))
    testPredictions <- exp(predict(model, testx, s = 5e-04))
...
form <- LogSalary ~ RegionReduced + SourceNameReduced + CategoryReduced + ContractTime + 
    ...
    ContractType:ContractTime + RegionReduced:CategoryReduced
...
```
##  [1] 8968 9041 8941 8978 8987 9014 8968 8995 8978 9011
##  Mean Training MAE: [1] 8988
##  [1] 9311 8875 9693 9252 8990 8977 9559 9194 9196 8888
##  Mean Test Set MAE: [1] 9194
```
...
text_data <- cbind(data, analysts, engineers, software, restaurant, worker, 
    days)

form <- LogSalary ~ RegionReduced + SourceNameReduced + CategoryReduced + ContractTime + 
    ContractType + CategoryReduced:ContractTime + CategoryReduced:ContractType + 
    ContractType:ContractTime + analysts + engineers + software + restaurant + 
    worker + days + CategoryReduced:RegionReduced
...
```
## Mean R.Squared:  0.3693 Mean Training MAE:  8911 Mean Test Set MAE:  9161
```
...
```
## [1] 8952 9004 8916
##  Mean Training MAE: [1] 8957
## [1] 9212 8887 9625
##  Mean Test Set MAE: [1] 9242
```
...
```
##  [1] "****"       "assistant"  "care"       "consultant" "developer" 
##  [6] "engineer"   "home"       "manager"    "nurse"      "sales"     
## [11] "senior"     "support"    "worker"
```
...
form <- LogSalary ~ analysts + engineers + software + restaurant + worker + 
    days + manage + senior + assist + develop + care + home

model <- hw2.cv.lm(form, text_data)
```

```
## Mean R.Squared:  0.2205 Mean Training MAE:  9697 Mean Test Set MAE:  9707
```
...
```
## [1] 9707 9719 9638 9731 9705
##  Mean Training MAE: [1] 9700
## [1]  9637  9521 10188  9477  9786
##  Mean Test Set MAE: [1] 9722
```
...
form <- LogSalary ~ RegionReduced + SubRegionReduced + SourceNameReduced + Category + 
    ContractTime + ContractType + CategoryReduced:ContractTime + CategoryReduced:ContractType + 
    ContractType:ContractTime + analysts + engineers + software + restaurant + 
    worker + days + manage + senior + assist + develop + care + home
...
```
## Mean R.Squared:  0.4438 Mean Training MAE:  8234 Mean Test Set MAE:  8364
```
...
```
##  [1] 8242 8273 8192 8239 8228 8274 8201 8248 8254 8273
##  Mean Training MAE: [1] 8242
##  [1] 8575 8133 8773 8317 8476 8092 8679 8311 8276 8044
##  Mean Test Set MAE: [1] 8368
```
...
submit <- cbind(submit, manage, senior, assist, develop, care, home, analysts, 
    engineers, software, restaurant, worker, days)

data <- text_data

form <- LogSalary ~ RegionReduced + SubRegionReduced + SourceNameReduced + CategoryReduced + 
    ContractTime + ContractType + CategoryReduced:ContractTime + CategoryReduced:ContractType + 
    ContractType:ContractTime + analysts + engineers + software + restaurant + 
    worker + days + manage + senior + assist + develop + care + home
...
form <- ~RegionReduced + SubRegionReduced + SourceNameReduced + CategoryReduced + 
    ContractTime + ContractType + CategoryReduced:ContractTime + CategoryReduced:ContractType + 
    ContractType:ContractTime + analysts + engineers + software + restaurant + 
    worker + days + manage + senior + assist + develop + care + home
...
submission <- data.frame(Id = data$Id, Salary = submitPredictions)
```
